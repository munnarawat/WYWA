...
const getUserProfile360 = async (req, res) => {
  try {
    ...
    // check student id are available
    ...
    // the masterMind security logic
    if (requesterRole === "thinkTank" && requesterRole === "student") {
    ...
    if (branchFilter.branch && targetUser.branch !== branchFilter.branch) {
      return res
        .status(403)
        .json({ success: false, message: "Unauthorized branch access." });
    }
    ...
    //  Dynamic Data Fetching (Role based);

    if (targetUser.role === "student") {
      // when target user is student get his attendance and library

      const [attendanceRecords, libraryRecords] = await Promise.all([
        AttendanceModel.find({ student: targetId }).sort({ date: -1 }).lean(),
        IssuedBook.find({ student: targetId })
          .populate("book", "title")
          .sort({ createdAt: -1 })
          .lean(),
      ]);
