...
const ProfileHeader = ({
  ...
  const displayImage = avatarPreview || authUser?.profile?.personal?.imageUrl;
  return (
    <motion.div
      ...
      <div className="bg-[#0d1117] rounded-[19px] px-7 py-8 relative overflow-hidden flex flex-col sm:flex-row items-center sm:items-start gap-6">
        ...
        />
        <div
          className="absolute -bottom-15 -left-15 w-52 h-52 pointer-events-none"
          style={{
            background:
        ...
        <div className="relative shrink-0">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(20,184,166,0.7), rgba(132,204,22,0.5))",
              WebkitMask:
          ...
          />
          <div className="w-22 h-22 rounded-full bg-[#131920] border border-teal-500/20 flex items-center justify-center text-3xl overflow-hidden">
            {displayImage ? (
              <img
                src={displayImage}
                alt="Profile"
          ...
          <motion.button
            ...
            className="absolute bottom-0 right-0 w-8 h-8 rounded-full flex items-center justify-center border-2 border-[#0d1117]"
        ...
        <div className="relative z-10 text-center sm:text-left flex-1">
          <h1
            className="font-extrabold text-[28px] mb-1.5 bg-clip-text text-transparent"
            style={{
              backgroundImage:
          ...
          <p className="text-[13px] text-slate-500 mb-3">
            {authUser?.branch && `${authUser.branch} Branch`}
          </p>
          <div className="flex flex-wrap justify-center sm:justify-start gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-wider text-slate-400">
              🎓 {authUser?.role || "User"}
            </span>
            ...
            {authUser?.profile?.academic?.batch && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                📅 Batch {authUser.profile.academic.batch}
              </span>
            )}
          </div>
        </div>
