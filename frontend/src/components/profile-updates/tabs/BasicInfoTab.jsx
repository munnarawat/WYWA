...
const BasicInfoTab = ({ register, errors, isEditMode }) => (
  <motion.div
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <InputField label="Gender" icon={<User />}>
        <select disabled={!isEditMode} {...register("personal.gender")}>
          <option value="" className="bg-[#0d1117] text-slate-100">
            Select Gender
          </option>
          <option value="Male" className="bg-[#0d1117] text-slate-100">
            Male
          </option>
          <option value="Female" className="bg-[#0d1117] text-slate-100">
            Female
          </option>
          <option value="Other" className="bg-[#0d1117] text-slate-100">
            Other
          </option>
        </select>
      </InputField>
      <InputField label="Blood Group" icon={<Droplet />}>
        <select disabled={!isEditMode} {...register("personal.bloodGroup")}>
          <option value="" className="bg-[#0d1117] text-slate-100">
            Select Blood Group
          </option>
          {BLOOD_GROUPS.map((bg) => (
            <option key={bg} value={bg} className="bg-[#0d1117] text-slate-100">
              {bg}
            </option>
          ))}
        </select>
