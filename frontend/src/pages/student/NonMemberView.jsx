  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 md:p-6 bg-[#070a0f] relative overflow-hidden">
      ...
      <motion.div
        ...
        className="relative w-full max-w-[560px] rounded-[28px] p-[1.5px]"
        style={{
          background: 'linear-gradient(135deg, rgba(20,184,166,0.6), rgba(255,255,255,0.06), rgba(132,204,22,0.5), rgba(99,102,241,0.3))',
          boxShadow: '0 0 80px rgba(20,184,166,0.08), 0 40px 80px rgba(0,0,0,0.5)',
        }}
      >
        <div
          className="rounded-[27px] px-8 py-12 md:px-12 text-center relative overflow-hidden"
          style={{ background: 'linear-gradient(160deg, #0e1520 0%, #080d14 60%, #060a10 100%)' }}
        >
          {/* Top Glow */}
          <div
          ...
          <motion.div variants={stagger} initial="hidden" animate="show">
            ...
            {/* Icon */}
            <motion.div variants={item} className="relative w-22 h-22 mx-auto mb-8">
              <motion.div
              ...
              />
              <div className="absolute inset-1.5 rounded-[18px] opacity-30"
                style={{ background: 'linear-gradient(135deg, #14b8a6, #84cc16)' }} />
              <div
                className="absolute inset-3 rounded-[14px] flex items-center justify-center"
                style={{
                  background: 'linear-gradient(135deg, #14b8a6, #10b981, #84cc16)',
            ...
            {/* Live Badge */}
            <motion.div variants={item} className="flex items-center justify-center gap-2 mb-5">
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-semibold tracking-widest uppercase"
            ...
            <motion.h2 variants={item}
              ...
              <span className="bg-clip-text text-transparent"
                style={{ backgroundImage: 'linear-gradient(135deg, #5eead4 0%, #14b8a6 40%, #84cc16 100%)' }}>
                Away From Home.
              </span>
            </motion.h2>

            {/* Body */}
            <motion.p variants={item} className="text-slate-500 text-[15px] leading-relaxed mb-5 max-w-[400px] mx-auto">
            ...
            <motion.div variants={item} className="flex justify-center mb-8">
              {[['500+', 'Members'], ['2', 'Cities'], ['5 yrs', 'Together']].map(([num, label], i) => (
                <div key={i} className={`px-6 ${i > 0 ? 'border-l border-white/[0.07]' : ''}`}>
                  <p className="text-[22px] font-extrabold bg-clip-text text-transparent"
                    style={{ backgroundImage: 'linear-gradient(135deg, #5eead4, #84cc16)' }}>{num}</p>
                  <p className="text-[11px] text-slate-600 font-medium uppercase tracking-widest mt-0.5">{label}</p>
                </div>
              ))}
            </motion.div>
            ...
            <motion.div variants={item} className="flex flex-wrap justify-center gap-2 mb-8">
              {[['🏔️', 'Stay Connected'], ['🤝', 'Brotherhood'], ['💼', 'Career Growth']].map(([icon, label]) => (
                <span key={label}
