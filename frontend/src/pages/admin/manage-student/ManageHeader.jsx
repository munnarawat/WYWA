...
const ManageHeader = ({ currentUser, filteredCount, searchQuery, setSearchQuery, stats }) => {
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 pb-6 border-b border-white/5">
        <div>
          ...
          <motion.h1
            ...
            style={{ backgroundImage: "linear-gradient(135deg, #f0fdf4 0%, #14b8a6 50%, #84cc16 100%)" }}
          >
            Manage Users
          </motion.h1>
          <motion.p
          ...
          >
            View, block, or promote users in <span className="text-slate-300 font-medium">{currentUser?.branch}</span> branch.
          </motion.p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="relative w-64">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex gap-3 flex-wrap">
        {[
          ...
          <div key={label} className="flex items-center gap-2 px-4 py-2 rounded-full text-[13px] text-slate-400 bg-white/5 border border-white/10">
